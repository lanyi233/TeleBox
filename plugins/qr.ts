...
async function ensureDependencies(): Promise<void> {
  ...
  if (missingDeps.length > 0) {
    ...
    throw new Error(`❌ 缺少依赖: ${missingDeps.join(', ')}<br><br>📦 安装命令:<br>${installCmd}<br><br>💡 安装完成后请重试`);
  }
}
      ...
      try {
        ...
        if (textToEncode) {
          ...
          try {
            ...
            await msg.edit({
              text: `❌ <b>生成二维码失败</b><br><br>${errorMsg.includes('❌') ? errorMsg : `<code>${htmlEscape(errorMsg)}</code>`}`,
              parseMode: 'html'
            });
          }
          return;
          ...
          try {
            ...
            if (decodedData.length > 0) {
              const resultText = decodedData
                ...
                .join('\n\n');
              await msg.edit({
                text: `✅ <b>成功解码二维码:</b><br><br>${resultText}`,
                parseMode: 'html'
              });
            } else {
              await msg.edit({
            ...
            const errorMsg = error.message || '未知错误';
            await msg.edit({
              text: `❌ <b>解码失败</b><br><br>${errorMsg.includes('❌') ? errorMsg : `<code>${htmlEscape(errorMsg)}</code>`}`,
              parseMode: 'html'
            });
          }
          return;
        ...
        const errorMsg = error.message || '未知错误';
        await msg.edit({
          text: `❌ <b>插件执行失败</b><br><br>${errorMsg.includes('❌') ? errorMsg : `<code>${htmlEscape(errorMsg)}</code>`}`,
          parseMode: 'html'
        });
      }
    }
