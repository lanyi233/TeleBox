...
• 平台限制：最多允许 120 张（超过将报错）`;
  ...
  private async handleCopyStickerSet(msg: Api.Message): Promise<void> {
    ...
    try {
      ...
      await msg.edit({
        text: `📦 找到贴纸包：${htmlEscape(originalSet.title)}<br>🎯 包含 ${stickers.length} 个贴纸<br><br>⏳ 开始复制贴纸包...`
      });
      ...
      if (stickers.length > maxStickers) {
        await msg.edit({
          text: `📦 贴纸包：${htmlEscape(originalSet.title)}<br>🎯 原包含 ${stickers.length} 个贴纸<br>⚠️ 为避免超时，将只复制前 ${maxStickers} 个贴纸（limit=${desired}，最大允许 120）<br><br>⏳ 开始处理贴纸...`
        });
      }
      ...
      for (let i = 0; i < processStickers.length; i++) {
        ...
        // 更新进度
        await msg.edit({
          text: `📦 贴纸包：${htmlEscape(originalSet.title)}<br>🎯 处理贴纸 ${i + 1}/${processStickers.length}...`
        });
      ...
      await msg.edit({
        text: `📦 贴纸包：${htmlEscape(originalSet.title)}<br>🎯 已处理 ${stickerInputs.length} 个贴纸<br><br>🚀 正在创建新贴纸包...`
      });
      ...
      try {
        ...
        if (result instanceof Api.messages.StickerSet) {
          ...
          await msg.edit({
            text: `✅ 贴纸包复制成功！<br><br>📦 原贴纸包：${htmlEscape(originalSet.title)}<br>🆕 新贴纸包：${htmlEscape(finalTitle)}<br>🔗 链接：${newSetUrl}<br>📊 贴纸数量：${stickerInputs.length}（limit=${desired}）<br><br>点击链接添加到 Telegram！`
          });
        } else {
          const prefixes = getPrefixes();
          await msg.edit({
