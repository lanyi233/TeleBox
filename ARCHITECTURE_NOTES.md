...
## 11. agent.ts “tsc 门禁”补全以 `: any` 收尾，反而削弱类型安全且半途而废（可维护性 / 风险）

- 问题描述：
  为推进第 9/10 条提出的 `tsc` 门禁，工作树中 `src/plugin/agent.ts` 被批量追加
  `: any` 形参/字段类型注解（约 200 处函数签名由 `function f(a, b)` 改为
  `function f(a: any, b: any)`）。本次健康检查已将该 WIP 收尾提交（commit `3bf4788`）。
  但这一做法存在设计缺陷：
    1. `: any` 只是把 `TS7006`（隐式 any）错误“显式化”，**并未真正收窄类型**，
       等于用 `any` 关闭了严格模式对这些位置的任何类型检查，与“让 `tsc` 成为门禁”
       的初衷背道而驰——门禁若依赖 `any` 填充，门禁本身已失效；
    2. 该补全**未做完**：提交后 `tsc --noEmit` 在 `agent.ts` 上仍有约 41 条真实错误
       （如 `config` 接口缺 `zn_name` 字段、`safeReply`/`safeEdit` 的 `options.html`/
       `plainFallback` 在 `{}` 类型上不存在、`right`/`left` 为 `unknown` 等），
       说明这是一轮跨多个 cron 周期、被反复中断的半成品；
    3. `any` 注解散落在 `agent.ts` 这个巨型文件里，后续若想真正修类型，还要先识别
       哪些是“为通过编译而加的 any”，哪些是“原本就明确的 any”，认知负担更重。
- 影响范围：可维护性（中 —— 半完成 + 真假难辨的 `any`，增加后续清理成本）；
  风险（低/中 —— `any` 关闭了类型检查，真实回归更易被静默放过，门禁形同虚设）。
- 建议改进方向：
  - 暂停“加 `: any` 凑编译”的做法，改为**真正的最小类型**：为 `provider`/`messages`/
    `context` 等核心参数补上准确的接口类型（或 `Record<string, unknown>` 代替 `any`）；
  - 把 `config` 的 `zn_name` 等字段补进 `agent_schema` 接口定义（或显式标注 `as any`
    收窄到单一取值点，而非整函数 `: any`）；
  - 若该文件确实长期作为转译产物维护（见第 9 条），则应承认它不在 `tsc` 门禁范围内，
    用 `// @ts-nocheck` 或移出 `src/`、单独打包，避免用 `: any` 假装它在门禁内；
  - 任何“修类型以过编译”的改动都应在同一次提交内让相关文件的 `tsc` 错误归零，
    杜绝跨周期的半成品留在工作树。